<template>
  <div class="page-container">
    ...
    <el-table :data="list" v-loading="loading" border>
      ...
      <el-table-column prop="noteType" label="分类" width="110" />
    ...
    <el-dialog v-model="detailVisible" title="话题详情" width="640px">
      <el-descriptions :column="1" border>
        <el-descriptions-item label="标题">{{ detail.title }}</el-descriptions-item>
        <el-descriptions-item label="作者">{{ detail.userName }} (ID: {{ detail.userId }})</el-descriptions-item>
        <el-descriptions-item label="分类">{{ detail.noteType || '—' }}</el-descriptions-item>
...
const load = async () => {
  loading.value = true
  try {
    const r = await adminApi.notes({ ...query, userId: query.userId || undefined, keyWord: query.keyWord || undefined })
